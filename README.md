...
## 蓝奏云下载地址
```
https://wwauw.lanzouu.com/iBK9H3mch4qf
```
## 作者抖音号
大号
...
## 2026/04/03
## 版本7.3.0
1.新增逻辑:如果检测到账号处于被风控状态(此时会无法参与福袋，登录此账号也将无法获取到福袋信息)，用于查找直播间的浏览器会自动切换到未登录状态(这是另一个独立的浏览器，不要登录！)

2.新增逻辑:如果你设置了不参与福袋，或者设置了满足了指定条件后不参与任何福袋，将不会再尝试获取福袋信息(这有利于更快解除风控状态)

**由于在风控状态下，账号中奖权重会大幅降低，所以请尽量设置不参与福袋！**

## 2026/03/29
## 版本7.2.0
1.新增了键盘监听事件’手动切换查找直播间的浏览器’，你可以按下’7’键以切换到另一个用于查找直播间的浏览器

2.现在用于查找直播间的浏览器会定时(每两小时)切换账号，以确保能比较稳定地调用接口
...
## 2026/03/11
## 版本7.0.0
**由于抖音官方更新，请停止使用版本6.1.0之前版本的程序！**

修复了由于抖音官方更新导致的福袋信息无法被获取到的bug

## 2026/02/28
## 版本6.1.0
1.优化了直播间存在多个红包时的处理逻辑

2.修改逻辑:功能性浏览器在停用状态下不会被打开/会自动关闭

3.修复了偶现的通过类别查找直播间的协程会停止工作的bug

4.修复了偶现的红包显示不完整但仍会参与该红包(实际上会参与不成功)的bug

5.修改了一些逻辑不合理的地方

## 2026/01/28
## 版本6.0.0
**由于抖音官方更新，请停止使用版本6.0.0之前版本的程序**

1.观察到跳转到新直播间后，如果有在上一个直播间参与过红包，且该红包倒计时未结束，则仍会等待该红包倒计时结束，已修复此bug

2.修复了经常性的可以打开红包但识别红包异常的bug(此bug最近才出现，疑似官方作了部分改动)

**严重bug:此bug容易导致频繁跳转直播间(这会忽略正在参与的其他红包)**

3.修复了在直播间同时存在红包和福袋的情况下，在参与红包后大概率获取不到红包开奖结果(如果最后中奖了)的bug

4.修复了在直播间有很多红包的情况下，容易参与红包异常的bug

5.修复了由于变量重置有误导致程序无法正常运行的bug

6.修改了一些逻辑不合理的地方

## 2026/01/25
## 版本5.2.0
1.新增了’设置代理’功能，你可以在’proxy.json’文件中设置代理相关信息，如何设置请参考’proxy.docx’这个文档

2.修复了偶现的重新进入意外退出的直播间后，会一直停留在该直播间的bug

3.修复了在直播间同时存在红包和福袋的情况下，首次参与红包会参与异常的bug

4.修复了在一些情况下，功能’符合条件时自动停止抢福袋’在日期发生变更时，仍会保持在已符合条件(若前一天到达了触发状态)的触发状态的bug
...
## 2026/01/01
## 版本5.1.1
1.功能’符合条件时自动停止抢福袋’新增了字段’连续未中粉丝团福袋的次数≥’

2.修改了’(单账号推荐设置)user.json’以及’(默认设置)user.json’的内容，现在的推荐设置相较于之前的设置更加合理

3.修复了在日期发生变更时，功能’符合条件时自动停止抢福袋’仍会保持在已符合条件(若前一天到达了触发状态)的触发状态的bug

4.修复了在开启’符合条件时自动抢福袋’功能后，设置’今日所有账号参与福袋数≥’字段值无效的bug

## 2025/12/27
## 版本5.1.0
1.功能’自动抢红包’新增了字段’自动停止该功能，如果连续未中红包的次数≥’

2.新增了无限制功能’符合条件时自动停止抢福袋’

你可以在’账号说明.docx’这个文档中看到关于这些功能的详细说明
...
## 2025/12/21
## 版本4.6.0
1.新增了键盘监听事件’停用/重新开启自动抢红包功能’，你可以按下’6’键以停用/重新开启自动抢红包功能(此监听事件仅VIP用户可触发)

2.新增逻辑:在开启自动抢红包功能的情况下，如果连续 30*账号数量 次未中红包，将自动停用自动抢红包功能

3.修复了在多开账号情况下可能会出现的一些bug(包括比较严重的bug:

①若两个账号同时中奖，账户余额会扣除异常(可能会少扣or多扣)

②浏览器出现异常行为的频次略高

③在参与红包的时候有时候会秒跳转到另一直播间，在这种情况下，如果中奖了，则会遗漏中奖信息)

4.修改了一些逻辑不合理的地方

5.考虑到服务器等多项成本问题以及免费礼物收益在总收益中的占比较大的情况，在此版本以及后续版本下会进行如下扣费:

**每获得价值10钻石的免费礼物，将会消耗0.01元的余额**

## 2025/12/17
## 版本4.5.1
1.修复了点赞操作会经常性导致福袋最终中奖概率统计失败的bug

2.修复了偶现的切换直播间后会意外进行点赞的bug，此bug容易导致一些中奖信息没有被统计进今日lottery_info中

3.修改逻辑:观察到当跳转到新的直播间等待福袋开奖时，如果这时候有可参与的人气红包，在参与该人气红包后，若福袋不符合条件，则会直接跳转到下一直播间，因此为了减少不必要的损失，在这种情况下不会参与人气红包

## 2025/12/12
## 版本4.5.0
1.新增了键盘监听事件’停用/重新启用功能性浏览器’，你可以按下’5’键以停用/重新启用功能性浏览器，功能性浏览器是指会自动向下滑动页面的浏览器，考虑到功能性浏览器起到的作用不大，友友们可自行选择是否启用功能性浏览器

2.新增逻辑:在参与福袋/红包之后有一定概率会给直播间点赞

3.优化了参与红包的逻辑

## 2025/12/04
## 版本4.4.0
1.新增了键盘监听事件’多开账号’，你可以按下’4’键以多开1个账号(为了保险起见，目前只支持多开1个账号)(此监听事件仅VIP用户可触发)

**如果你的电脑配置不高，不建议使用此功能！此外，该功能不大稳定，谨慎使用**

2.修复了程序暂停时会阻塞页面加载的bug
...
## 2025/11/18
## 版本4.2.1
1.user.json新增了’昵称’字段，你可以修改该字段的值以改变你的账号的昵称，
你的昵称会被用于排行榜以及后续的相关事项上

2.修复了’数据可视化’功能更新余额不正确的bug

3.修复了偶现的浏览器刷新页面/回到上一个页面超时导致程序无法正常运行的bug
...
## 2025/11/12
## 版本4.1.0
1.VIP由时间制改为余额制，原先购买过时间制VIP的用户不会受影响(已购买的时间制VIP过期后才会消耗余额)

如果你的账号有余额，你可以正常使用VIP功能，每获得10钻石，将会消耗0.03元的余额，时间制VIP过期且余额不足的用户不会消耗余额

2.你可以前往[bailanjo.cn](https://bailanjo.cn/)登录你的账号并绑定QQ邮箱，这样可以正常使用签到功能，每天签到可随机获得[0.020, 0.050]元的余额

3.减少了程序异常重启的发生频率

## 2025/11/05
## 版本4.0.3
1.更换了服务器，现在基本不会出现连接服务器失败的情况了。

**由于服务器变更，如果你是VIP用户，请首先运行’迁移数据’文件夹中的’迁移数据.exe’(请先修改’迁移数据’->_internal->迁移.json文件里的username)，否则将无法正常使用VIP功能！！！**

2.修复了偶现的不参与福袋/红包的浏览器遇到未知错误会重启失败的bug

3.修改了一些逻辑不合理的地方

## 2025/11/02
## 版本4.0.2
1.新增逻辑:由于倒计时<10s的红包不能直接抢，因此碰到倒计时<10s的红包时会等待’抢’字样出现后再抢

2.由于开启’自动抢红包’功能后，抢红包的频率过高，因此降低了参与红包的优先度

3.减少了偶现的浏览器频繁重启事件的发生次数

4.修复了在协程中错误使用同步锁导致偶现的程序运行异常的bug

## 2025/10/28
## 版本4.0.1
1.提高了方法’通过类别查询直播间’的成功率

2.新增逻辑:每隔2小时会自动重启不参与福袋/红包的浏览器

3.修复了VIP用户在20:00-22:00之间登录时，22:00后VIP功能无法正常使用的bug
...
## 2025/10/21
## 版本3.3.2
1.新增逻辑:如果有正在参与的剩余时间<30s的红包，将不会跳转到新直播间

2.修复了search线程在选择不想要某种类型的福袋条件下经常会运行异常的bug

3.如果’今日总收益’<0，推送消息的查看详情中的’今日总收益’会出现’+-’号，此bug已修复
